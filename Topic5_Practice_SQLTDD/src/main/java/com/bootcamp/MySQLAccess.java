...
public class MySQLAccess {
	...
	public void writeResultSetQueryKey4(ResultSet resultSet)
			throws SQLException {
	...
	public void writeResultSetQueryKey5(ResultSet resultSet)
			throws SQLException {
		if (resultSet.next()) {
			//print the firts line
			System.out.println("Results ------ Query 5");
			String student_first = resultSet.getString(3);
			...
			System.out.println("Final Note: " + resultSet.getInt(5));
			while (resultSet.next()) {
				// it is possible to get the columns via name
				// also possible to get the columns via the column number
				// which starts at 1
				// e.g., resultSet.getSTring(2);
				student_last = resultSet.getString(2);
				student_first = resultSet.getString(3);
				String name_course = resultSet.getString(4);
				int final_note = resultSet.getInt(5);
	...
	public void writeResultSetQueryKey6(ResultSet resultSet, int total_student_in_course)
			throws SQLException {
		if (resultSet.next()) {
			//print the firts line
	...
	public void writeResultSetQueryKey7(ResultSet resultSet)
			throws SQLException {
		if (resultSet.next()) {
			//print the firts line
